...
| **T2 — Endpoint de estorno** | `POST /{proto}/itens/{item}/estornar`: exige dispensação registrada do próprio CNPJ; `dispensado → estornado` com **reposição de saldo Σ**; **estorno = registro append-only próprio** (referencia `dispensacao_id`) — nunca UPDATE/DELETE em `dispensacoes`; evento novo (Fase 0.2); atualizar CLAUDE.md §2 e §5a no mesmo PR | core (ledger + estados) | M |
