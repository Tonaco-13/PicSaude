...
| B2 ✔ **EXECUTADO 31/08** | **§4 do GP-3** — histórico COMO × O QUE (o que vai a público no git) | **Intermediário** (recomendado): `git filter-repo` removeu `planejamento/` do histórico + filtrou a string do CNS antigo — histórico de commits/tickets PRESERVADO, shas mudaram (confirmado: `main` pós-B2 não tem ancestralidade comum com o `main` pré-B2; CNS antigo e `planejamento/` ausentes de `git log --all`). Engenheiro re-clonou no aviso do arquiteto. **Verdade honesta**: force-push limpa a main, mas blobs antigos podem seguir acessíveis nas páginas de PR do GitHub — purga total exige suporte do GitHub ou recriação do repo. GP-2 (curadoria da fachada) é o próximo passo |
...
| 8 | G2 talão digital — ✔️ **#226** (`8e1241e`, 30/08) → G3 talões na vitrine — ✔️ **#228** (`f747cf1`, 30/08) | ✔️ fechada |
