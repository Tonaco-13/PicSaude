...
4. **Posse é exclusiva**: pedido já com laboratório (status `agendado`) → 409;
   terminal → 422.
5. **Fila do laboratório** (`GET /dispensadores/fila-exames`): o dono vê, o não-dono
   NÃO vê, e o EX-custodiante deixa de ver (custódia atual, nunca histórica).
...
def test_fila_nao_vaza_entre_laboratorios(client, seed_usuario, seed_paciente):
    token_p = obter_token_prescritor(client, seed_usuario)
    proto = _emitir_pedido(client, token_p)
    assert _transferir(client, proto, _CNPJ_LAB_A).status_code == 201

    protos_b = {p["protocolo"] for p in _fila(client, _CNPJ_LAB_B)}
    assert proto not in protos_b, "VAZAMENTO: laboratório B enxergou exame do laboratório A"


def test_ex_custodiante_sai_da_fila(client, outer_conn, seed_usuario, seed_paciente):
    """Custódia ATUAL, nunca histórica — quem perdeu a posse some da fila.

    A segunda transferência é escrita direto na cadeia de custódia (e não por um
    endpoint) de propósito: hoje nenhum caminho de produto move um pedido já
    `agendado` de um laboratório para outro — `/agendar` exige itens `pendente` e
    `/transferir-laboratorio` exige o pedido `emitido`. O que este teste guarda é
    a QUERY da fila: ela lê o último elo da cadeia, não "qualquer elo".
    """
    token_p = obter_token_prescritor(client, seed_usuario)
    proto = _emitir_pedido(client, token_p)
    assert _transferir(client, proto, _CNPJ_LAB_A).status_code == 201
    ...
    with outer_conn.cursor() as cur:
        cur.execute(
            ...
            "SELECT id, NULL, %s, %s, %s, %s FROM pedidos_exame WHERE protocolo = %s",
            (_CNPJ_LAB_A, _CNPJ_LAB_B, "2026-01-01T00:00:00", "{}", proto),
        )

    assert proto not in {p["protocolo"] for p in _fila(client, _CNPJ_LAB_A)}
    assert proto in {p["protocolo"] for p in _fila(client, _CNPJ_LAB_B)}
...
def test_fila_exames_admin_sem_cnpj_leva_422(client, seed_usuario, seed_paciente):
    r = client.get("/dispensadores/fila-exames", headers=_headers(_token("00000000000", "admin")))
    assert r.status_code == 422, r.text
